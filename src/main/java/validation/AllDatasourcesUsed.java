...
public class AllDatasourcesUsed implements ValidationRuleInterface{

	@Override
	public String getName() {
		// TODO Auto-generated method stub
		return "Requested Databases Not Influencing Imported Files";
	}
	...
	public List getViolations(String[] args, Model model) {
      //  String fileIRI = args [0];
		...
		String query = Constants.PREFIXES + " SELECT DISTINCT ?database ?databaseL ?file  WHERE {?database rdfs:label ?databaseL.  ?database a shp:Database. ?linkagePlan a shp:DataLinkagePlan; schema:exifData ?dataSource. ?dataSource shp:database ?database. NOT EXISTS { ?file a shp:DataSet; prov:wasDerivedFrom ?database.}  }";
	...
	public JPanel getSimpleResult(String[] args, Model model) {
		...
		ArrayList<Database> variablesList =  (ArrayList<Database>) new AllDatasourcesUsed ().getViolations(args, model);
